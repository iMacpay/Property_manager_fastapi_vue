<template>
	<div>
		...
		<el-table :data="tableData" border style="width: 100%">
			...
			<el-table-column label="应缴电费(元)">
				<template scope="scope">
					<span style="color: #67C23A;" v-if="scope.row.ispay!==1">{{scope.row.money}}</span>
					<span style="color: #E6A23C;" v-if="scope.row.ispay==1">{{scope.row.hadpay}}</span>
				</template>
			</el-table-column>
			<el-table-column prop="date" label="用电月份">
			</el-table-column>
		...
		<el-dialog :title="digTitle" :visible.sync="centerDialogVisible" width="30%" center>
			<el-form :rules="rule" status-icon :model="fromname" class="demo-ruleForm" ref="fromname">
				<el-form-item label="户主账号:" prop="uptele" v-if="this.digTitle=='新增电费信息'">
					<el-input placeholder="请输入户主账号" v-model.number="fromname.uptele"></el-input>
				</el-form-item>
				<el-form-item label="用电量(度):" prop="upnum">
					<el-input placeholder="请输入用电量" v-model.number="fromname.upnum" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="用电月份:" prop="update" v-if="this.digTitle=='新增电费信息'">
...
<script>
	export default {
		...
		mounted() {
			this.$axios.get(this.url + "/condelfee", {
				params: {
					page_index: 1,
				}
			}).then((res) => {
		...
		methods: {
			...
			eleDelete(id) {
				this.$confirm('您确定要删除此条数据吗?', '提示', {
					...
					this.$axios.post(this.url + "/deelfee", {
						efid: id
					}).then((res) => {
						if(res.data.code=="0")
						{
			...
			addEle() {
				...
				this.digTitle = "新增电费信息"
			},
			updateConfirm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						if (this.digTitle == "新增电费信息") {
							this.$axios.post(this.url + "/addelfee", {
								ard: {
									utele: this.fromname.uptele
								},
								args: {
									efdate: this.fromname.update,
									efnum: this.fromname.upnum
								}
							}).then((res) => {
								if (res.data.code == 1) {
									this.$message({
						...
						if (this.digTitle == "修改电费信息") {
							this.$axios.post(this.url + "/updelfee", {
								efid: this.fromname.upid,
								efnum: this.fromname.upnum
							}).then((res) => {
								if (res.data.code == "0") {
									this.$message({
