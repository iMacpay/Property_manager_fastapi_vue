<template>
	<div>
		...
		<el-table :data="tableData" border style="width: 100%">
			...
			<el-table-column prop="measure" label="住房面积(㎡)">
			</el-table-column>
			<el-table-column label="应缴物业费(元)">
				<template scope="scope">
					<span v-if="scope.row.ispay!==1">{{scope.row.money}}</span>
					<span v-if="scope.row.ispay==1">{{scope.row.hadpay}}</span>
				</template>
			</el-table-column>
			<el-table-column prop="date" label="物业费月份">
			</el-table-column>
		...
		<el-dialog :title="digTitle" :visible.sync="centerDialogVisible" width="30%" center>
			<el-form :rules="rule" status-icon :model="fromname" class="demo-ruleForm" ref="fromname">
				<el-form-item label="户主账号:" prop="uptele">
					<el-input placeholder="请输入户主账号" v-model.number="fromname.uptele"></el-input>
				</el-form-item>
				<el-form-item label="物业费月份:" prop="update">
...
<script>
	export default {
		...
		mounted() {
			this.$axios.get(this.url + "/condprfee", {
				params: {
					page_index: 1,
				}
			}).then((res) => {
		...
		methods: {
			...
			eleDelete(id) {
				this.$confirm('您确定要删除此条数据吗?', '提示', {
					...
					this.$axios.post(this.url + "/delprfee", {
						pfid: id
					}).then((res) => {
						if(res.data.code=="0")
						{
			...
			addEle() {
				...
				this.digTitle = "新增物业费信息"
			},
			updateConfirm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						if (this.digTitle == "新增物业费信息") {
							this.$axios.post(this.url + "/addprfee", {
								ard: {
									utele: this.fromname.uptele
								},
								args: {
									pfdate: this.fromname.update,
								}
							}).then((res) => {
								if (res.data.code == 1) {
									this.$message({
